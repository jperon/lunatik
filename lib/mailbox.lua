...
function MailBox:receive(timeout)
	...
	elseif header_size < sizeoft then
		error("malformed message: header too small")
	end

	local message_size = string.unpack("T", header)
	local message, actual_size = queue:pop(message_size)

	if actual_size < message_size then
		error("malformed message: content smaller than expected")
	end
	return message
end
